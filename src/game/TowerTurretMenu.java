...
public class TowerTurretMenu extends Tower 
{

	public TowerTurretMenu(GameState state, GameControl control, int x, int y) {
	...
	public void update(double timeElapsed) {
		...
		if (state.getMouseClicked()) {
			...
			if (dX < 40 && dY < 40) {
				state.consumeClick();
				state.addAnimatable(new TowerTurretMoving(state, control, state.getMouseX(), state.getMouseY()));
			}
		}
	}
